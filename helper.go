...
func scanRows(rows *sql.Rows, dest any) error {
	...
	for rows.Next() {
		...
		// fieldForCol[j] is set when column j maps to a struct field
		// whose type (or *type) implements sql.Scanner — in that case
		// we scan straight into the field via its own Scan method
		// instead of through an `any` + reflect.Convert, which cannot
		// handle types like sql.NullString/sql.NullFloat64.
		fieldForCol := make([]reflect.Value, len(columns))
		...
		for i, col := range columns {
			...
			if f.CanSet() && f.CanAddr() && reflect.PtrTo(f.Type()).Implements(scannerType) {
				fieldForCol[i] = f
		...
		for j, col := range columns {
			if fieldForCol[j].IsValid() {
				// already scanned directly into the field above
				continue
			}
