...
func (c *CRUD) Create(model any) (string, error) {
	...
	// set timestamps if model supports it
...
// Delete removes a record physically, unless the model implements SoftDeleter,
// in which case it sets the soft-delete column to the current timestamp instead.
func (c *CRUD) Delete(model any, field string, value any) error {
	...
	// soft delete — update the designated column instead of deleting
...
func buildMeta(t reflect.Type) *modelMeta {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		// fallback: if no column name in tag, derive it from the field name
		if col == "" {
	...
	// fallback primary key: first field named "id" or "ID"
	if meta.primaryKey == nil {
